...
- A common ledger accounts for polling and screen/power boundaries. Plug/unplug events override a stale sticky battery intent until a matching battery reading arrives, preventing Android’s broadcast ordering from leaving the wrong power state in effect. Monotonic elapsed time drives session duration; stopping monitoring freezes it, and resetting a paused session leaves it at zero. Powered and unknown-power intervals do not contribute to discharge buckets.
...
Android 8 device logs also exposed a rejected initial battery broadcast: the compatibility implementation of a non-exported receiver adds an app-only permission, while Android delivers a sticky replay without a sender UID. The tracker now registers its fixed set of platform-protected system actions without that incompatible permission. Android still rejects attempts by ordinary apps to send those protected actions. Pause uses the foreground broadcast queue so a user tap is not held behind unrelated background receivers. The disposable API 26 CI emulator additionally disables its broken bundled Messages RCS component, whose crash dialog and stalled broadcast had interrupted tests; this setup is gated to CI emulators and never changes a physical phone.

The follow-up adds regression cases for all-screen-on sessions, missing/reset counters, valid zero drain, pause/reset timing, absent system fields, plugged-but-not-charging status, Doze semantics and notification behavior. Local verification of these corrections is recorded below; final device runs and APK identity are recorded in PR #7.
...
Follow-up production revision `ef2f792`: a clean local build passed all 144 unit tests, debug and instrumentation APK assembly, the optimized release build, and lint with zero errors (143 warnings). GitHub CI and APK builds also passed. API 26 passed all 10 device tests with zero skips. The notification screenshot helper additionally waits for the actual expanded notification so captured images can verify its details and Pause button. Final API 26/API 36 runs, reviewed screenshots, and the exact downloadable APK are recorded in PR #7.
...
## Follow-up: preferred notification and BatStats overhead

The notification presentation has been returned to its earlier compact format. The ordinary monitor again uses the fixed Monitoring battery title with a level/current/voltage line; its detailed form keeps the same summary-first layout. Current direction is written explicitly as in/out, missing sensors remain unavailable, charging-paused remains distinct from charging, and the notification is no longer presented as a newly timed event on every update. The drain notification again uses the emoji state title, compact On/Off line, section dividers and Reset action. Its corrected body omits the invalid Active/Idle split and does not assign drain rates or energy to CPU awake/deep-sleep time.

The monitoring service now reuses the repository's user-selected battery sampling stream instead of maintaining a second one-minute timer and duplicate gauge read. Screen/power broadcasts retain exact state boundaries, while persisted screen-off sampling is capped at once per minute instead of honoring wasteful five- or ten-second intervals. Plain-notification rendering ignores unrelated drain-state emissions and unrelated settings changes. The expensive cumulative `batterystats --checkin` per-app poll runs every 15 minutes rather than every five (four instead of twelve dumps per hour), while later deltas still include activity between polls. Usage Events are not queried while charging or screen-off, unchanged widget payloads are not sent back through AppWidgetManager, and service startup no longer races the application-level retention cleanup. These changes remove known work; they are not a claim of a measured battery-life percentage until profiled on physical hardware.

Per-app energy remains Android's battery-accounting estimate, and shared UIDs cannot be split exactly among packages. Screen-on/off energy depends on the device's charge counter; CPU sleep/awake durations do not establish separate energy measurements. No measured FPS, memory or battery-life improvement is claimed without device profiling. Release APKs remain unsigned when no release keystore is supplied; debug APKs are installable under `org.mlm.batstats.debug`. CI creates a fresh debug signing key, so it cannot replace a debug APK signed by an earlier runner. Export data before uninstalling the older debug build, then install and import to retain history.
